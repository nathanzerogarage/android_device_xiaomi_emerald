...
class XiaomiTanzaniteUdfpsHandler : public UdfpsHandler {
  public:
    void init(fingerprint_device_t* device) {
        ...
        std::thread([this]() {
            ...
            while (true) {
                ...
                LOG(DEBUG) << "localHbmUiReady: " << localHbmUiReady;
    ...
    void onFingerDown(uint32_t x, uint32_t y, float /*minor*/, float /*major*/) {
        ...
        // Set up HBM and force-enable FOD
        setFingerDown(true);
    }

    void onFingerUp() {
        LOG(DEBUG) << __func__;
        // Set up HBM and force-disable FOD
        setFingerDown(false);
    }
    ...
    void onAcquired(int32_t result, int32_t vendorCode) {
        ...
        if (result != FINGERPRINT_ACQUIRED_VENDOR) {
            // Disable FOD unconditionally to avoid stuck HBM
            setFodMode(false);
            setHighlight(false);
